...
rank-$d$ update to the whole Green's function, where $d$ is the rank of the
vertex. That update is a level-2 BLAS / LAPACK operation: it streams
$N_{\text{dim}}^2$ complex entries to carry out
$\mathcal{O}(d \, N_{\text{dim}}^2)$ floating-point operations. At large
dimensions of the Green's function, the Green's function updates on spin flip
acceptance become bound by memory bandwidth rather than simply by arithmetic,
and this will cause performance degradation in most typical use cases. To
alleviate this, we can exploit the fact that at the level of the spin flip
proposals we do not require the updated Green's function to be stored as a
full matrix. Instead, we just need the $d \times d$ block $G(P,P)$
(where $P$ references \texttt{Op\%P}) to form the acceptance ratio,
and, on subsequent acceptance of a proposal, $d$ rows and $d$ columns.
Therefore, following Ref.~\cite{Sun2024}, we can hold the Green's function in a
``factorised'' form
\begin{equation}
  G = G_{\text{stale}} + X Y^{T},
  \qquad X, Y \in \mathbb{C}^{N_{\text{dim}} \times k},
\end{equation}
...
$\mathcal{O}(N_{\text{dim}} d k)$, respectively. Once we have accumulated $k$
columns
the panels are flushed into
$G_{\text{stale}}$ with a single \texttt{ZGEMM} (a level-3 operation).

We choose the factorised region to span a single time slice's sequential
vertex loop. \texttt{Wrapgr\_mod} opens the Green's function after the hopping
propagator has been
applied and closes it before the slice is left, so the panels are flushed once
per slice and the stabilization, measurements, global updates and any
time-displaced calculations are all handed an ordinary Green's function. Between
...
\end{equation}
the conjugation thus passes through the factorization and is applied
to the panels by \texttt{Op\_Wrap\_panels} at a cost that does not scale with
$N_{\text{dim}}$.

It should be noted that the scheme reassociates the arithmetic (which will
  introduce a relative numerical error given ALF is typically
compiled with a \texttt{fast-math} flag) but is in principle deterministic,
so reruns at the \emph{same} delay depth will produce the same Markov chain,
and any configuration written to \texttt{confout} is unchanged. The only
exception is of course if a flip proposal happens
to lie within rounding of its acceptance threshold and therefore branching can
occur -- a Metropolis ``near-tie''.
...
\begin{itemize}
  ...
  \item \texttt{auto} -- the depth is measured at start-up.  For each candidate
    $k$ the cost of one flush and one panel reconstruction is timed at the
    $N_{\text{dim}}$ of the model at hand, and the largest candidate within
    5\% of the cheapest cost is taken (overshooting tends to perform better
    on average).  The result depends on the machine and on the loaded
    BLAS / LAPACK library;
  \item \texttt{formula} -- the closed form $k \approx \sqrt{2 N_{\text{dim}}}$
    is used without timing anything.  This is also the automatic probe's
    ...
    function flush.
\end{itemize}
For \texttt{auto} and \texttt{formula} the depth is clamped to a
range $[8, 256]$ derived from benchmarks across a range of models.

The chain's delayed depth and how it was arrived at
(\texttt{off}, \texttt{fixed}, \texttt{probe} or \texttt{formula}) are written
to
each run's \texttt{info} file as \texttt{Delay depth} and
