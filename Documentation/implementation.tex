...
\begin{table}[h!]
   ...
   \begin{tabular}{@{} l l @{$\;\;$} l @{}}\toprule
    ...
    \texttt{Latt\%nnlist}                              & \texttt{int}   &  Nearest neighbor indices: $j = \texttt{nnlist} (i, n_1, n_2) $, \\
                                                       &                &  $|n_1|, |n_2| \leq  \texttt{nnlist\_range} $, $\vec{R}_j = \vec{R}_i + n_1 \vec{a}_1  + n_2 \vec{a}_2 $ \\
   ...
   The \hl{highlighted} variables must be specified by the user.  Other components of \texttt{Lattice} are generated upon calling: \texttt{Call Make\_Lattice(L1, L2, a1, a2, Latt,nnlist\_range)}.   The variable \texttt{nnlist\_range} is optional and defaults to unity.
    \label{table:lattice}}
\end{center}
\end{table}
%
