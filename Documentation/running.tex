...
\begin{table}[h!]
	\begin{center}
		\begin{tabular}{@{} p{0.22\linewidth} p{0.74\linewidth} @{}}\toprule
			...
			\texttt{ALF\_SHM\_CHUNK\_SIZE\_GB} & An environment variable that sets the chunk size in GBs for the memory shared between different MPI processes on the same computing node. By default it is zero (i.e., no sharing), but can be set to, e.g., 1.0 or 2.0 GB or larger if, for instance, the total number of MPI communicators is so large as to trigger MPI error messages. \\
			\texttt{ALF\_DELAY\_K} & An environment variable that switches on the delayed update of the Green's function (see Sec.~\ref{sec:delayed}) and sets its delay depth $k$. Unset or \texttt{0} keeps the immediate update; a positive integer fixes $k$; \texttt{auto} measures a depth at start-up and \texttt{formula} takes it from a closed form. Worth trying for large $N_{\mathrm{dim}}$, where the immediate update is bound by memory bandwidth. \\\bottomrule
		\end{tabular}
